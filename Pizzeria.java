...
public class Pizzeria {
	...
	private static int inicioTurnoTarde = 00;
	...
	public Pizzeria(String noum, String address) {
		...
		this.address = address;
		workers = null;
		foods = new ArrayList<ComidaPizzeria>();
		orders = null;
	}
	...
	public List<Integer> VisualizarListaComidas(List<ComidaPizzeria> platos, String seccion) {
		...
		for(int i=0; i < platos.size();i++) {
			if(platos.get(i).getSection() == seccion){
				System.out.print(i + ":" + platos.get(i).toString());
				opciones.add(i);	//añade a la lista de Integer el número de orden del producto en la lista de comidas
			}
		};
		return opciones;
	}
	...
	public int seleccionaSeccion(String strSeccion) {
		...
		System.out.println("Seleccione la sección. -1 para volver al menú superior.");
		do {
			try {
	...
	public void VzzisualizarListaComidasPedido(List<Integer> platos) {
		for (Integer p : platos) {
			Iterator<ComidaPizzeria> it = foods.iterator();
			ComidaPizzeria cm = new ComidaPizzeria();
			do {
				cm = it.next();
				if (cm.getFoodId() == p) {
