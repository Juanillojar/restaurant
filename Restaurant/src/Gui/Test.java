...
public class Test {
	...
	public static void main(String[] args) {
		...
		if (!conex.isTimeOut()) { // connected to database motor
			...
			{ // no conneted to database
				...
				if (JOptionPane.showConfirmDialog(null, "Database not found. Create database?", "Database not found",
						JOptionPane.YES_NO_CANCEL_OPTION) == 0) {// create database and tables

					conex.createDatabase(arrayConfig);
					conex.createTables(arrayConfig);
					conex.setConnected(true);
					JOptionPane.showMessageDialog(null, "Database created successfully", "Information",
							JOptionPane.INFORMATION_MESSAGE);
					if (JOptionPane.showConfirmDialog(null, "Do you want to insert exmample data?", "Question",
							JOptionPane.OK_CANCEL_OPTION) == 0) {// insert example data in database
			...
			if (JOptionPane.showConfirmDialog(null, "Do you want to charge example data?",
					"No conecction to database motor", JOptionPane.YES_NO_CANCEL_OPTION) == 0) {// insert data into
																								// memory structures
				createProducts(productsRestaurant);
				createWorkers(workers);
				createPedidos(ordersRestaurant, productsRestaurant, workers);
	...
	public static SecretKeySpec createKey() {
		...
		try {
			return createSecretKey(passwordKey.toCharArray(), salt, iteractionCount, KeyLength);
	...
	public static void createProducts(List<Productos> lista) {
		...
		Productos plato2 = new Productos("Pollo milanesa", Section.STARTERS,
				"Pollo, tomate, cebolla, pan rallado, Huevo, pimienta", 10.50, false);
		Productos plato3 = new Productos("Aros de cebolla", Section.STARTERS, "Cebolla, aceite, pan rallado", 5.50,
				false);
		Productos plato4 = new Productos("Musaka", Section.STARTERS, "Berengena, queso, tomate, lecha, harina, sal",
				11.50, false);
		Productos plato5 = new Productos("Tabla patés", Section.STARTERS, "Pate de higado, paté de atún", 7.20, true);
		Productos plato6 = new Productos("Macarrones boloñesa", Section.PASTAS, "Pasta, Tomate, Cebolla, champiñones",
				7.5, false);
		Productos plato7 = new Productos("Pizza Caprichosa", Section.PIZZAS,
				"Harina, aceite, agua, tomate, Queso, Cebolla, champiñones", 10, true);
		Productos plato8 = new Productos("Pizza Margarita", Section.PIZZAS, "Harina, aceite, agua, tomate, Queso", 9.80,
				false);
		Productos plato9 = new Productos("Pizza Pepperoni", Section.PIZZAS, "Tomate, Queso, pepperoni, salchicas", 8.60,
				false);
		Productos plato10 = new Productos("Ensalada Tropical", Section.SALADS,
				"Lechuga, tomate, gauda, piña, manzana, kiwi, salsa rosa", 9.20, false);
		Productos plato11 = new Productos("Ensalada Nostra", Section.SALADS,
				"Lechuga, tomate, zanahoria, atún, maiz, espárragos", 8.00, false);
		...
		Productos plato17 = new Productos("Lomo y Huevos", Section.COMBINADOS, "Cerdo, huevo, patata, aceite, sal",
				9.90, false);
		Productos plato18 = new Productos("Cazón y Huevos", Section.COMBINADOS, "Cazón, huevo, patata, aceite, sal",
				9.90, false);
		Productos plato19 = new Productos("Salchichas y pimientos", Section.COMBINADOS,
				"Cerdo, pimiento, patata, aceite, sal", 5.90, false);
		Productos plato20 = new Productos("Hamburguesa casa", Section.OTHERS, "Cerdo, huevo, patata, acerite, sal",
				9.90, false);
	...
	public static void createWorkers(List<Trabajador> workers) {
		...
		// Creación de los arrays con los idiomas que conoce cada camarero, los
		// camareros, repartidores y cocineros
	...
	public static void createPedidos(List<Pedido> orders, List<Productos> productsList, List<Trabajador> workersList) {
		// Creación de la lista de comidas para pedido1 y el objeto pedido1 y la lista
		// de objetos pedido.
